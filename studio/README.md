...
Written **automatically**: as the last stage of every production run (Produce preview, Produce automatically, Render finals) when `publish.json` is missing or older than `episode.json`, and by the Publish tab itself when it opens without a kit. `node scripts/kit.mjs <id> [--force]` does the same from the command line. **Regenerate kit** rewrites it on demand. Nobody has to type titles or descriptions; every field stays editable.
...
- **direct**: uploaded **as soon as the entry is added**, private, with the platform's own publish time (YouTube `publishAt`; Facebook `scheduled_publish_time`, which must be 10 minutes to 30 days ahead), and marked `published` with the remote URL and `scheduledNatively: true`. The video goes live at the slot on the platform, so this computer does not need to be on then. Entries the platform cannot hold (a Facebook slot more than 30 days out, or one added while the connection was broken) wait for the scheduler, which uploads them within their lead time as a fallback. `Publish now` uploads immediately as public.
