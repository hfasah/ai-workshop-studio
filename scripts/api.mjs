...
// Per-platform kit (scripts/publish.mjs): JSON mode, limits enforced in code, written to publish.json and publish.md.
...
app.get("/api/episodes", (_req, res) => {
  const schedule = pub.readSchedule();
  const list = episodeIds().map((id) => {
    ...
    return {
      ...
      schedule: pub.nextForEpisode(id, schedule),
    };
  });
  res.json(list);
});
...
app.get("/api/episodes/:id", requireEpisode, (req, res) => {
  ...
  res.json({id, episode, status, stage: episodeStage(id, status), docs: listDocs(id), build: buildSummary(id), outputs: listOutputs(id), cast: CAST, job: active ? publicJob(active) : null, publishKit: pub.readKit(id), schedule: pub.readSchedule().filter((e) => e.episodeId === id).map(pub.publicEntry)});
});

// MP4s under out/<id>/ with dimensions (ffprobe when available) for the Publish tab's asset selectors.
app.get("/api/episodes/:id/assets", requireEpisode, (req, res) => res.json(pub.listAssets(req.params.id)));

// The saved kit, or a hand-edited one (every field editable in the UI; PUT stores the edited kit as publish.json + publish.md).
app.get("/api/episodes/:id/publish-kit", requireEpisode, (req, res) => {
  const kit = pub.readKit(req.params.id);
  if (!kit) return res.status(404).json({error: "No publish.json yet. Generate the kit first."});
  res.json(kit);
});
app.put("/api/episodes/:id/publish-kit", requireEpisode, (req, res) => {
  const {id} = req.params;
  const b = req.body;
  if (!b || typeof b !== "object" || Array.isArray(b)) return res.status(400).json({error: "Body must be the kit object"});
  const prev = pub.readKit(id);
  const kit = pub.finalizeKit(b, pub.kitContext(id));
  res.json(pub.writeKit(id, kit, {engine: prev?.engine ?? "edited", model: prev?.model ?? "Hippolyte"}));
});
...
// Publishing kit: the text engine → episodes/<id>/publish.json + publish.md. ?dryRun=1 returns {system, prompt} instead of calling the model.
app.post("/api/episodes/:id/publish-kit", requireEpisode, (req, res) => {
  const {id} = req.params;
  if (req.query.dryRun) {
    const e = getEngine();
    const {system, prompt} = pub.kitPrompt(id);
    return res.json({dryRun: true, engine: {name: e.name, model: e.model}, system, prompt, legacy: buildPublishPrompt(id).prompt});
  }
  const running = activeJobFor(id);
  if (running) return res.status(409).json({error: `A job is already running for ${id}: ${running.label}`, job: publicJob(running)});
...
// ---------- publishing: connections, schedule, scheduler (scripts/publish.mjs) ----------
const fail = (res, e, code = 400) => res.status(code).json({error: String(e.message ?? e)});
const connectionsInfo = () => ({...pub.publicConnections(PORT), platforms: pub.PLATFORMS, vias: pub.VIAS, statuses: pub.STATUSES, fixedTags: pub.FIXED_TAGS, defaults: pub.defaultSlots()});

app.get("/api/connections", (_req, res) => res.json(connectionsInfo()));

app.put("/api/connections", (req, res) => {
  try {
    pub.applyConnectionsPatch(req.body ?? {});
...
// Lists the channel / page / Blotato accounts without posting anything.
app.post("/api/connections/:platform/test", async (req, res) => {
  const fn = {youtube: pub.youtubeTest, facebook: pub.facebookTest, blotato: pub.blotatoTest}[req.params.platform];
  if (!fn) return res.status(404).json({error: "platform must be youtube, facebook or blotato"});
  try {
    res.json({...(await fn()), connections: connectionsInfo()});
  } catch (e) {
    res.status(502).json({error: String(e.message ?? e), connections: connectionsInfo()});
  }
});

app.get("/api/connections/youtube/auth-url", (_req, res) => {
  try {
    res.json({url: pub.youtubeAuthUrl(PORT), redirectUri: pub.redirectUri(PORT)});
  } catch (e) {
    fail(res, e);
  }
});
...
app.get("/api/connections/youtube/callback", async (req, res) => {
  const page = (title, body) => res.type("html").send(`<!doctype html><meta charset="utf-8"><title>${title}</title><body style="font-family:Inter,system-ui,sans-serif;padding:40px;max-width:560px"><h2>${title}</h2><p>${body}</p><p><a href="/publishing">Back to the Studio</a></p>`);
  if (req.query.error) return page("YouTube not connected", `Google returned: ${String(req.query.error)}`);
  try {
    const r = await pub.youtubeCallback({code: String(req.query.code ?? ""), state: String(req.query.state ?? ""), port: PORT});
    page("YouTube connected", `Channel: <strong>${r.channelTitle || "(name not read yet: click Test in the Studio)"}</strong>. You can close this tab.`);
  } catch (e) {
    page("YouTube not connected", String(e.message ?? e));
  }
});

app.get("/api/schedule", (_req, res) => res.json(pub.readSchedule().map(pub.publicEntry).sort((a, b) => Date.parse(a.scheduledAt) - Date.parse(b.scheduledAt))));

app.post("/api/schedule", async (req, res) => {
  try {
    res.status(201).json(pub.publicEntry(await pub.createEntry(req.body ?? {}, {port: PORT})));
  } catch (e) {
    fail(res, e);
  }
});
...
app.put("/api/schedule/:id", async (req, res) => {
  try {
    res.json(pub.publicEntry(await pub.updateEntry(req.params.id, req.body ?? {}, {port: PORT})));
  } catch (e) {
    fail(res, e, /No such/.test(String(e.message)) ? 404 : 400);
  }
});

app.delete("/api/schedule/:id", (req, res) => {
  try {
    res.json(pub.deleteEntry(req.params.id));
  } catch (e) {
    fail(res, e, /No such/.test(String(e.message)) ? 404 : 409);
  }
});

// Uploads now (direct / Blotato) or marks a manual entry due. Only entries in schedule.json can ever be published.
app.post("/api/schedule/:id/publish-now", async (req, res) => {
  try {
    res.json(pub.publicEntry(await pub.publishEntry(req.params.id, {now: true})));
  } catch (e) {
    fail(res, e, /No such/.test(String(e.message)) ? 404 : 409);
  }
});

app.post("/api/schedule/:id/posted", (req, res) => {
  try {
    res.json(pub.publicEntry(pub.markPosted(req.params.id, req.body?.url)));
  } catch (e) {
    fail(res, e, /No such/.test(String(e.message)) ? 404 : 400);
  }
});
...
app.get("/api/publish-log", (req, res) => {
  const n = Math.min(Number(req.query.limit ?? 100), 1000);
  const lines = exists(pub.LOG_PATH) ? fs.readFileSync(pub.LOG_PATH, "utf8").trim().split("\n").filter(Boolean) : [];
  const parse = (l) => {
    try {
      return JSON.parse(l);
    } catch {
  ...
  res.json(lines.slice(-n).map(parse));
});
...
app.listen(PORT, () => {
  ...
  // Publishing scheduler: every 60 s, uploads direct entries within their lead time and marks manual entries due.
  if (process.env.STUDIO_SCHEDULER !== "off") {
    pub.startScheduler(60000);
    console.log(`Publishing scheduler on (studio/schedule.json, ${pub.readSchedule().length} entries). Set STUDIO_SCHEDULER=off to disable.`);
  }
});
